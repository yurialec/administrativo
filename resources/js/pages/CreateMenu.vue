<template>
    <section class="menu-form-page">
        ...
        <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-4">
            <div>
                <h1 class="h4 mb-0">Cadastrar Menu</h1>
            </div>
            <router-link
        ...
        <form class="row g-4" @submit.prevent="store">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-md-6">
                                ...
                                <select id="menuParent" v-model="menu.parent_id" class="form-select">
                                    ...
                                    <option v-for="item in menus" :key="item.id" :value="item.id">
                                        {{ item.title }}
                                    </option>
                                </select>
                            </div>
...
export default {
    ...
    methods: {
        async store() {
            ...
            try {
                const response = await axios.post('/admin/menus/store', this.createRequestPayload());
        ...
        async search() {
            ...
            try {
                const response = await axios.get('/admin/menus/list-to-create');
        ...
        createRequestPayload() {
            return {
                ...this.menu,
                children: this.menu.children.map(child => ({
                    title: child.title,
                    icon: child.icon,
                    route: child.route
                }))
